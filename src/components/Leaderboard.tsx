...
const Leaderboard: React.FC<LeaderboardProps> = ({ players }) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-10 w-full max-w-sm border border-slate-50" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        <div className="w-10 h-10 bg-amber-50 rounded-xl flex items-center justify-center text-amber-500">
          <Trophy size={20} />
        </div>
        <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tighter">{t('arenaRanks')}</h3>
      </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex flex-col">
                ...
                {player.streak && player.streak > 1 && (
                  <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest">🔥 {player.streak} {t('streak')}</span>
                )}
              </div>
            </div>
